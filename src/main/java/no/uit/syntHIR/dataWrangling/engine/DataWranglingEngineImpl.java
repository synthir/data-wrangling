...
public class DataWranglingEngineImpl implements DataWranglingEngine {
	...
	public List<ResourceResList> convertCSVToFHIRResourcesJson(BufferedReader csvFileBufferedReader) {
		...
		try {
			...
			// Rename the headers of the CSV according to the mapping given in the mapping
			// file (mapping.yaml)

			{

				// Get list of headers from the input CSV file
				String[] headers = csvFileBufferedReader.readLine().split(",");

				// Load the file from resources
				Resource resource = resourceLoader.getResource("classpath:mapping.yaml");

				// Get list of new headers by comparing with mapping file
				List<String> newHeaderList = DataWranglingUtil.compareWithMappingFileAndReturnNewHeaderList(
						resource.getInputStream(), Arrays.asList(headers));

				// Overwrite the existing headers in the CSV
				{
					// Read the existing data, including the headers
					CSVReader csvReader = null;
					try {
						csvReader = new CSVReader(csvFileBufferedReader);
						List<String[]> csvData = csvReader.readAll();
						csvData.set(0, newHeaderList.toArray(new String[newHeaderList.size()]));
						newCSVBufferedReader = DataWranglingUtil.writeListOfStringArrayToCSV(csvData);
			...
			combinedNPRNorPDPojos = DataWranglingUtil.readCsvToBeanList(newCSVBufferedReader,
					CombinedNPRNorPDPojo.class,
			...
			for (CombinedNPRNorPDPojo combinedNPRNorPDPojo : combinedNPRNorPDPojos) {
				...
				String hospitalizationDischargeLocation = combinedNPRNorPDPojo
						.getHospitalizationPatientDischargeLocation();
				...
				String numberOfPackagesDispensedForDrugDDD = combinedNPRNorPDPojo
						.getNumberOfPackagesDispensedForDrugDDD();
				// String reimbursementLegalCategory =
				// combinedNPRNorPDPojo.getReimbursementLegalCategory();

				/*
				 * For each record in the NPR and NorPD dataset, first create Patient and
				...
				if (!(BasicUtil.isStringNull(patientCountyNumber) || BasicUtil.isStringBlank(patientCountyNumber)
						|| BasicUtil.isStringEmpty(patientCountyNumber)))
				...
				if (!(BasicUtil.isStringNull(patientDeathYear) || BasicUtil.isStringBlank(patientDeathYear)
						|| BasicUtil.isStringEmpty(patientDeathYear))) {
					if (!(BasicUtil.isStringNull(patientDeathMonth) || BasicUtil.isStringBlank(patientDeathMonth)
							|| BasicUtil.isStringEmpty(patientDeathMonth))) {
				...
				if (!(BasicUtil.isStringNull(practitionerDateOfBirth)
						|| BasicUtil.isStringBlank(practitionerDateOfBirth)
						|| BasicUtil.isStringEmpty(practitionerDateOfBirth))) {
				...
				// Convert Json to Resource DTO object
				PractitionerRes practitionerRes = objectMapper.readValue(practitionerJsonFHIRRes,
						PractitionerRes.class);
				...
				{
					String levelOfCare = "";
					if (!(BasicUtil.isStringNull(hospitalizationLevelOfCare)
							|| BasicUtil.isStringBlank(hospitalizationLevelOfCare)
							|| BasicUtil.isStringEmpty(hospitalizationLevelOfCare)))
...

				{
					...
					if (!(BasicUtil.isStringNull(hospitalizationUrgency)
							|| BasicUtil.isStringBlank(hospitalizationUrgency)
							|| BasicUtil.isStringEmpty(hospitalizationUrgency)))
				...
				if (!(BasicUtil.isStringNull(hospitalizationPatientVisitStart)
						|| BasicUtil.isStringBlank(hospitalizationPatientVisitStart)
						|| BasicUtil.isStringEmpty(hospitalizationPatientVisitStart)) &&
						!(BasicUtil.isStringNull(hospitalizationPatientVisitEnd)
								|| BasicUtil.isStringBlank(hospitalizationPatientVisitEnd)
								|| BasicUtil.isStringEmpty(hospitalizationPatientVisitEnd))) {

					DateTime startDate = BasicUtil.convertStringDateToDateTimeFormat(hospitalizationPatientVisitStart,
							BasicConstants.HOSPITALIZATION_START_END_DATE_FORMAT);
					DateTime endDate = BasicUtil.convertStringDateToDateTimeFormat(hospitalizationPatientVisitEnd,
							BasicConstants.HOSPITALIZATION_START_END_DATE_FORMAT);

					fhirResourceEncounter
							.setHospitalizationVisitLength(BasicUtil.differenceBetweenDates(startDate, endDate));
				}
...

				{
					...
					if (!(BasicUtil.isStringNull(hospitalizationDischargeLocation)
							|| BasicUtil.isStringBlank(hospitalizationDischargeLocation)
							|| BasicUtil.isStringEmpty(hospitalizationDischargeLocation))) {

						int dischargeLocInt = Integer.parseInt(hospitalizationDischargeLocation);

						fhirCodeDischargeLocation = (dischargeLocInt == 1 ? "Other healthcare facility"
								: dischargeLocInt == 2 ? "Skilled nursing facility" : "Other");
						dischargeLocation = (dischargeLocInt == 1 ? "Another Medical Institution"
								: dischargeLocInt == 2 ? "Nursing homes, a municipal Unit" : "Others");
					}
				...
				// Populate MedicationRequest, Medication and MedicationDispense which has
...

				{
					...
					if (!(BasicUtil.isStringNull(reimbursementICDCode) || BasicUtil.isStringBlank(reimbursementICDCode)
							|| BasicUtil.isStringEmpty(reimbursementICDCode)))
				...
				if (!(BasicUtil.isStringNull(prescribedDrugATCCodeDDD)
						|| BasicUtil.isStringBlank(prescribedDrugATCCodeDDD)
						|| BasicUtil.isStringEmpty(prescribedDrugATCCodeDDD))) {
				...
				// Convert Json to Resource DTO object
				MedicationRequestRes medicationRequestRes = objectMapper.readValue(medicationRequestJsonFHIRRes,
						MedicationRequestRes.class);
				...
				if (!(BasicUtil.isStringNull(numberOfPackagesDispensedForDrug)
						|| BasicUtil.isStringBlank(numberOfPackagesDispensedForDrug)
						|| BasicUtil.isStringEmpty(numberOfPackagesDispensedForDrug))) {

					String formattedNumberOfPackagesDispensed = BasicUtil
							.replaceDotWithComma(numberOfPackagesDispensedForDrug);
					fhirResourceMedicationDispense.setMedicationPackagesDispensed(formattedNumberOfPackagesDispensed);

				}

				if (!(BasicUtil.isStringNull(numberOfPackagesDispensedForDrugDDD)
						|| BasicUtil.isStringBlank(numberOfPackagesDispensedForDrugDDD)
						|| BasicUtil.isStringEmpty(numberOfPackagesDispensedForDrugDDD))) {

					String formattedNumberOfPackagesDispensedDDD = BasicUtil
							.replaceDotWithComma(numberOfPackagesDispensedForDrugDDD);
					fhirResourceMedicationDispense
							.setDddMedicationPackagesDispensed(formattedNumberOfPackagesDispensedDDD);

				}

				fhirResourceMedicationDispense.setDayYearOfDispense(prescribedDrugDeliveryDate); // update with
																									// anonymized
																									// dispensed day
																									// which is negative
				...
				// Convert Json to Resource DTO object
				MedicationDispenseRes medicationDispenseRes = objectMapper.readValue(medicationDispenseJsonFHIRRes,
						MedicationDispenseRes.class);
	...
	public InputStream convertFHIRResourcesJsonToCSV(List<List<String>> fhirResourceRecords) {
		...
		for (List<String> fhirResourceRecord : fhirResourceRecords) {
			...
			for (String fhirResourceRecordStr : fhirResourceRecord) {
				...
				if (!(BasicUtil.isStringNull(fhirResourceRecordStr) || BasicUtil.isStringBlank(fhirResourceRecordStr)
						|| BasicUtil.isStringEmpty(fhirResourceRecordStr))) {
					...
					try {
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_PATIENT)) {
							...
							String patientGender = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("gender"));
							String patientDateOfBirth = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("birthDate"));
							String patientIdentifierValue = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("value"));
							String patientIdentifierType = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("type").get("text"));

							// Empty check on death date time, since it can be empty
							String patientDeathDateTime = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("deceasedDateTime"));

							String patientPostalCode = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("address").get(0).get("postalCode"));
							String patientCity = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("address").get(0).get("city"));
							// String patientCountry =
							// jsonObjNode.get("address").get(0).get("country").textValue();
							...
							if (patientAgeGroupExtensionObj != null) {
								...
								if (patientAgeGroupExtensionObj != null) {
									...
									String patientAgeGroup = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
											patientAgeGroupExtensionObj.get("valueString"));
							...
							if (!(BasicUtil.isStringNull(patientDeathDateTime)
									|| BasicUtil.isStringBlank(patientDeathDateTime)
									|| BasicUtil.isStringEmpty(patientDeathDateTime))) {

								DateTime patientDeathDateTimeDt = BasicUtil
										.convertStringDateToDateTimeFormat(patientDeathDateTime, "yyyy-MM");
								combinedNPRNorPDPojo
										.setPatientDeathMonth(String.valueOf(patientDeathDateTimeDt.getMonthOfYear()));
								combinedNPRNorPDPojo
										.setPatientDeathYear(String.valueOf(patientDeathDateTimeDt.getYear()));
							}
						}
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_PRACTITIONER)) {
							...
							// Empty and null check on Gender and date of birth, since these can have
							// negative values

							String practitionerGender = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("gender"));
							String practitionerDateOfBirth = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("birthDate"));
							String practitionerIdentifierValue = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("value"));
							String practitionerIdentifierType = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("type").get("text"));
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_ENCOUNTER)) {
							...
							if (!(BasicUtil.isStringNull(levelOfCare) || BasicUtil.isStringBlank(levelOfCare)
									|| BasicUtil.isStringEmpty(levelOfCare)))
								levelOfCare = levelOfCare.equals("in-progress") ? "1" : "2";

							String levelOfUrgency = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("priority").get("coding").get(0).get("code"));
							if (!(BasicUtil.isStringNull(levelOfUrgency) || BasicUtil.isStringBlank(levelOfUrgency)
									|| BasicUtil.isStringEmpty(levelOfUrgency)))
								levelOfUrgency = levelOfUrgency.equals("EMER") ? "1" : "2";

							String hospitalizationStartDate = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("period").get("start"));
							String hospitalizationEndDate = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("period").get("end"));

							String hospitalizationDischargeLocation = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("hospitalization").get("dischargeDisposition").get("text"));

							if (!(BasicUtil.isStringNull(hospitalizationDischargeLocation)
									|| BasicUtil.isStringBlank(hospitalizationDischargeLocation)
									|| BasicUtil.isStringEmpty(hospitalizationDischargeLocation)))
								hospitalizationDischargeLocation = (hospitalizationDischargeLocation
										.equals("Institusjon or Sykehjem, kommunal enhet") ? "1"
												: hospitalizationDischargeLocation
														.equals("Nursing homes, a municipal Unit") ? "2" : "3");
							...
							combinedNPRNorPDPojo
									.setHospitalizationPatientDischargeLocation(hospitalizationDischargeLocation);
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_MEDICATION)) {
							...
							String prescribedDrugNumber = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("value"));
							String prescribedDrugATCCode = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("code").get("coding").get(0).get("code"));
							String prescribedDrugName = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("code").get("text"));
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_MEDICATION_REQUEST)) {
							...
							String prescriptionNumber = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("identifier").get(0).get("value"));
							String prescriptionLegalCategory = BasicUtil
									.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("category").get(0).get("text"));
							;
							String prescriptionLegalCategoryCode = BasicUtil.checkIfJsonNodeObjectValueIsEmpty(
									jsonObjNode.get("category").get(0).get("coding").get(0).get("code"));
							...
							combinedNPRNorPDPojo.setReimbursementLegalCategory(
									BasicUtil.checkIfJsonNodeObjectValueIsEmpty(reimburesementLegalCategoryObj));
							combinedNPRNorPDPojo.setReimbursementLegalCategoryCode(
									BasicUtil.checkIfJsonNodeObjectValueIsEmpty(reimbursementLegalCategoryCodeObj));

							String reimbursementICDCode = "";
							String reimbursementICPCCode = "";
							{
								String reimbursementICDIPCPCCode = BasicUtil
										.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("note").get(2).get("text"));

								if (!(BasicUtil.isStringNull(reimbursementICDIPCPCCode)
										|| BasicUtil.isStringBlank(reimbursementICDIPCPCCode)
										|| BasicUtil.isStringEmpty(reimbursementICDIPCPCCode))) {
							...
							JsonNode prescribedDrugATCCodeDDDObj = jsonObjNode.get("dosageInstruction").get(0)
									.get("doseAndRate").get(0).get("doseQuantity");
							JsonNode prescribedDrugATCCodeDDDDoseObj = jsonObjNode.get("dosageInstruction").get(0)
									.get("doseAndRate").get(0).get("doseQuantity");

							if (prescribedDrugATCCodeDDDObj != null)
								combinedNPRNorPDPojo.setPrescribedDrugATCCodeDDD(
										prescribedDrugATCCodeDDDObj.get("value").toString());

							if (prescribedDrugATCCodeDDDDoseObj != null)
								combinedNPRNorPDPojo.setPrescribedDrugATCCodeDDDDose(
										prescribedDrugATCCodeDDDDoseObj.get("unit").textValue());
						}
						...
						if (fhirResourceType.equalsIgnoreCase(APIConstants.FHIR_RESOURCE_MEDICATION_DISPENSE)) {
							...
							combinedNPRNorPDPojo.setPrescribedDrugDeliveryDate(
									BasicUtil.checkIfJsonNodeObjectValueIsEmpty(jsonObjNode.get("whenHandedOver")));
							combinedNPRNorPDPojo.setNumberOfPackagesDispensedForDrug(
									numberOfPackagesDispensedForDrugObj.toString());

							if (numberOfPackagesDispensedForDrugDDDObj != null)
								combinedNPRNorPDPojo.setNumberOfPackagesDispensedForDrugDDD(
										numberOfPackagesDispensedForDrugDDDObj.get("value").toString());
						}
		...
		String csvFileLocation = "combined_registry_data_for_SDG.csv";
		InputStream convertedCSVFileInputStream = null;
		...
		try {
			...
			convertedCSVFileInputStream = new FileInputStream(csvFile);
		...
		return convertedCSVFileInputStream;
	}

	/**
	 * Converts the list of FHIR resources (List<ResourceResList>)
	 * to CSV file. It reads the each FHIR resource JSON object,
	 * identifies the resource type and maps it the corresponding
	 * field of the CSV file.
	 * 
	 * 
	 * @param List of Json objects as List<ResourceResList>
	...
	public InputStream convertFHIRResourcesJsonObjToCSV(List<ResourceResList> resourceResponseList) {
		...
		for (ResourceResList resourceResponseObj : resourceResponseList) {
			...
			// Null/Empty/Blank check on patient death date-time
			if (!(BasicUtil.isStringBlank(patientDeathDateTime)
					|| BasicUtil.isStringEmpty(patientDeathDateTime)
					|| BasicUtil.isStringNull(patientDeathDateTime))) {

				DateTime patientDeathDateTimeDt = BasicUtil.convertStringDateToDateTimeFormat(patientDeathDateTime,
						"yyyy-MM");
			...
			String hospitalizationDischargeLocation = encounterRes.getHospitalization().getDischargeDisposition()
					.getText();

			if (!(BasicUtil.isStringBlank(hospitalizationDischargeLocation)
					|| BasicUtil.isStringEmpty(hospitalizationDischargeLocation)
					|| BasicUtil.isStringNull(hospitalizationDischargeLocation)))
				hospitalizationDischargeLocation = (hospitalizationDischargeLocation
						.equals("Institusjon or Sykehjem, kommunal enhet") ? "1"
								: hospitalizationDischargeLocation.equals("Nursing homes, a municipal Unit") ? "2"
										: "3");
			...
			combinedNPRNorPDPojo.setPrescriptionLegalCategoryCode(
					medicationRequestRes.getCategory().get(0).getCoding().get(0).getCode());
			...
			combinedNPRNorPDPojo.setReimbursementICDCode(reimbursementICDCode);
			combinedNPRNorPDPojo.setReimbursementICPCCode(reimbursementICPCCode);
			combinedNPRNorPDPojo.setPrescribedDrugATCCodeDDD(String.valueOf(medicationRequestRes.getDosageInstruction()
					.get(0).getDoseAndRate().get(0).getDoseQuantity().getValue()));
			combinedNPRNorPDPojo.setPrescribedDrugATCCodeDDDDose(medicationRequestRes.getDosageInstruction().get(0)
			...
			combinedNPRNorPDPojo.setNumberOfPackagesDispensedForDrug(
					String.valueOf(medicationDispenseRes.getQuantity().getValue()));
			combinedNPRNorPDPojo.setNumberOfPackagesDispensedForDrugDDD(
					String.valueOf(medicationDispenseRes.getDaysSupply().getValue()));
		...
		// Write to CSV and upload CSV file to Azure storage
		String csvFileLocation = "csv_file_from_FHIR_resources.csv";
		// ByteArrayOutputStream fileOutStreamFromStorage = null;

		FileInputStream convertedCSVFileInputStream = null;
		File csvFile = null;
		try {
			// create CSV
			csvFile = new File(csvFileLocation);
			csvFile = DataWranglingUtil.readBeanToCSV(csvFile, CombinedNPRNorPDPojo.class, combinedNPRNorPDPojoList);
			convertedCSVFileInputStream = new FileInputStream(csvFile);
		} catch (IOException e1) {
			e1.printStackTrace();
